...
def generate(path, output_dir, keep_going):
    ...
    retcode = subprocess.Popen(["cmake", "-DCMAKE_VERBOSE_MAKEFILE=ON", "-DOUTPUT_DIR:STRING=" + output_dir, "."], cwd=path).wait()
...
def make(path, keep_going):
    if keep_going:
        subprocess.Popen(["make", "-j96", "-k"], cwd=path).wait()
    else:
        retcode = subprocess.Popen(["make", "-j96"], cwd=path).wait()
        if retcode != 0:
